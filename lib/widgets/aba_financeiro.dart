...
class _AbaFinanceiroState extends State<AbaFinanceiro> {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _sectionTitle(context, 'Fechamentos — Últimos 6 Meses'),
          const SizedBox(height: 4),
          Text(
  ...
  Widget _buildTaxaCard(int fechados, int perdidos, int pipeline, ColorScheme cs) {
  ...
  // ── Gráfico de fechamentos por mês ───────────────────────────────────────
  Widget _buildFechamentosPorMes(ColorScheme cs) {
    final agora = DateTime.now();
    final mesesRef = List.generate(6, (i) {
      final m = agora.month - 5 + i;
